...
setMethod("model1", signature(object = "SLURM"), function(object)
  {
    ...
    for (i in 1:N)
    {
      ...
      {
        jobID = jobID + 1
        fname=paste(file.path(object@path,"submit","batch-model1"),jobID,".slurm",sep="",collapse="")
        sink(fname)
        cat("cd norm/results\n")
      }
...
setMethod("model2", signature(object = "SLURM"), function(object)
  {
    ...
    else
    {
      ...
      {
        fname=paste(file.path(object@path,"submit","model2-batch-job"),i,".slurm",sep="",collapse="")
        sink(fname)
        cat("#!/bin/bash\n")
        for (j in 1:nrun)
        {
...
setMethod("quant", signature(object = "SLURM"), function(object)
  {
    ...
    for (i in 1:N)
    {
      ...
      {
        jobID = jobID + 1
        fname=paste(file.path(object@path,"submit","quant"),jobID,".slerm",sep="",collapse="")
        sink(fname)
    ...
    else
    {
      ...
      for (i in c(1:object@totalJobs))
      {
        fname=paste(file.path(object@path,"submit","quant"),i,".slurm",sep="",collapse="")
        sink(fname)
        cat("#!/bin/bash\n")
        for (j in 1:nrun)
        {
          idx = idx + 1
          cat(sprintf("sh plots/quant-job%d.sh\n",idx))
        }
        if (extra > 0)
        {
          idx = idx + 1
